...
test_script = """
echo "=== TEST 1: Try login with 100 (Baytak User) on at.mt.mtapp.ly (MUST BE REJECTED) ==="
curl -s -X POST http://127.0.0.1:3005/api/auth/login \
  -H "Content-Type: application/json" \
  -H "Host: at.mt.mtapp.ly" \
  -H "x-tenant-slug: at.mt" \
  -d '{"employeeCode":"100","pinCode":"0000"}'
echo ""
echo "=== TEST 2: Try login with 101 on at.mt.mtapp.ly (MUST SUCCEED AS MADAR TECH ADMIN) ==="
curl -s -X POST http://127.0.0.1:3005/api/auth/login \
  -H "Content-Type: application/json" \
  -H "Host: at.mt.mtapp.ly" \
  -H "x-tenant-slug: at.mt" \
