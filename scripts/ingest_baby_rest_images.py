...
images = [
    {
        ...
        "productName": "بيبي ريست قطرات للرضع 15 مل (Baby Rest Drops)",
        "caption": "نجرب في برنامج😅 - صورة صنف بيبي ريست",
    ...
    },
    {
        ...
        "productName": "BABY REST ORAL DROPS 15ml",
        "caption": "صورة العلبة باللغة الإنجليزية - Baby Rest Oral Drops 15ml",
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            INSERT INTO "WhatsAppShortageRequest" (
                ...
                '{b64_str}',
                'IMAGE',
                'تم التقاط الصورة من مجموعة صيدلية بيتك بنجاح 📸',
                NOW(),
                NOW()
            );
            """
            
            stdin, stdout, stderr = c.exec_command(f'docker exec -i hodoork_postgres psql -U postgres -d hodoork_db')
            stdin.write(sql)
            stdin.flush()
            stdin.channel.shutdown_write()
            out = stdout.read().decode()
            err = stderr.read().decode()
            print(f"Inserted {img['productName']}: OUT={out.strip()} ERR={err.strip()}")
