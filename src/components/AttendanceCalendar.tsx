        ...
        {selectedDayRecords.length === 0 ? (
          ...
          <div className="space-y-6">
              ...
              <div className="bg-slate-900/95 text-white rounded-3xl p-5 sm:p-6 border border-slate-800 shadow-xl space-y-5 overflow-x-auto">
                ...
                {/* 1. Master Combined Coverage Timeline Bar (شريط التغطية الموحد لكافة الموظفين - سطر موحد للموظف وبسمك رفيع 50%) */}
                <div className="p-3 bg-slate-950/90 rounded-2xl border border-blue-500/40 shadow-inner flex flex-wrap sm:flex-nowrap items-start gap-4">
                  <div className="w-full sm:w-56 shrink-0 space-y-0.5 text-xs pt-0.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-md font-extrabold bg-blue-500/20 text-blue-300 border border-blue-500/30">
                        مدمج ({groupedList.length} موظفين)
                      </span>
                    </div>
                    <div className="text-[10px] text-slate-400 font-semibold">
                      سطر موحد لكل موظف بخطوط رفيعة (سمك 50%)
                    </div>
                  </div>

                  <div className="relative flex-1 bg-slate-900/90 rounded-xl border border-slate-700/80 p-2 overflow-hidden min-w-[300px]" dir="ltr">
                    ...
                    {/* Employee Sub-tracks: One Row Per Employee with all their shifts in that row, slim 12px height (h-3) */}
                    <div className="relative z-10 space-y-1">
                      {groupedList.map((emp) => (
                        <div key={`master-row-${emp.userId}`} className="relative h-3 w-full">
                          {emp.records.map((rec) => {
                            const pos = getShiftBarPosition(rec.checkInTime, rec.checkOutTime);
                            return (
                              <div
                                key={`master-shift-${rec.id}`}
                                style={{ left: pos.left, width: pos.width }}
                                className={`absolute inset-y-0 bg-gradient-to-r ${emp.theme.bg} text-white text-[9px] font-black font-mono rounded-sm flex items-center justify-between px-1.5 shadow-sm transition-all whitespace-nowrap overflow-hidden`}
