        ...
        {selectedDayRecords.length === 0 ? (
          ...
          <div className="space-y-6">
              ...
              <div className="bg-slate-900/95 text-white rounded-3xl p-5 sm:p-6 border border-slate-800 shadow-xl space-y-5 overflow-x-auto">
                ...
                {/* 1. Master Combined Coverage Timeline Bar (شريط التغطية الموحد لكافة الموظفين - خطوط مكدسة تحت بعضها) */}
                <div className="p-4 bg-slate-950/90 rounded-2xl border border-blue-500/40 shadow-inner flex flex-wrap sm:flex-nowrap items-start gap-4">
                  <div className="w-full sm:w-56 shrink-0 space-y-1 text-xs pt-1">
                    ...
                    <div className="text-[10px] text-slate-400 font-semibold">
                      خطوط الدوام تحت بعضها لكافة الموظفين لرؤية التغطية والتداخلات لليوم
                    </div>
                  </div>
                  ...
                  <div className="relative flex-1 bg-slate-900/90 rounded-xl border border-slate-700/80 p-2 overflow-hidden min-w-[300px]" dir="ltr">
                    ...
                    {/* Stacked Sub-tracks for Each Employee Shift under each other */}
                    <div className="relative z-10 space-y-1.5">
                      {groupedList.flatMap((emp) =>
                        emp.records.map((rec) => {
                          const pos = getShiftBarPosition(rec.checkInTime, rec.checkOutTime);
                          return (
                            <div key={`master-${rec.id}`} className="relative h-6 w-full">
                              <div
                                style={{ left: pos.left, width: pos.width }}
                                className={`absolute inset-y-0 bg-gradient-to-r ${emp.theme.bg} text-white text-[10px] font-black font-mono rounded-md flex items-center justify-between px-2.5 shadow-sm transition-all whitespace-nowrap overflow-hidden`}
